...
export const WorldMapChart: React.FC<WorldMapChartProps> = ({ data, isLoading = false }) => {
  const [geoCountries, setGeoCountries] = React.useState<string[]>([]);

  React.useEffect(() => {
    fetch(geoUrl)
      .then((res) => res.json())
      .then((geoData) => {
        const names = geoData.features.map((feature: any) => feature.properties.NAME);
        setGeoCountries(names);
      });
  }, []);

  const normalizedData = React.useMemo(() => {
    if (!data || geoCountries.length === 0) return {};
